...
class ChildHealthListRow(models.Model):
    ...
    @staticmethod
    def fetch(query: Optional[str] = None, limit: int = 50, offset: int = 0) -> List[Dict[str, Any]]:
        """
        SELECT * FROM view_all_child_health_record(%s, %s, %s)
        """
        try:
            with connection.cursor() as cur:
                cur.execute(
                    ...
                    SELECT
                      ...
                      tt_status_date,
                      created_at
                    FROM view_all_child_health_record(%s::TEXT, %s::INT, %s::INT)
                    """,
                    [query, limit, offset],
                )
                cols = [c[0] for c in cur.description]
    ...
    @staticmethod
    def count(query: Optional[str] = None) -> int:
        """
        ...
        try:
            with connection.cursor() as cur:
                cur.execute(
                    ...
                    SELECT COUNT(*) FROM (
                      SELECT 1
                      FROM view_all_child_health_record(%s::TEXT, 500000::INT, 0::INT)
                    ) t
                    """,
                    [query],
                )
